...
if [ -f "$CONFIGFILE" ]; then
        ...
        echo ""
        echo " » Die Konfigurationsdatei '$CONFIGFILE' ist nicht lesbar!"
        echo " +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
...
# Uninstall-Skript        #
###########################
mkdir -p /home/"$BENUTZERNAME"/"$NEXTCLOUDDNS"/Nextcloud-Installationsskript/
touch /home/"$BENUTZERNAME"/"$NEXTCLOUDDNS"/Nextcloud-Installationsskript/uninstall.sh
cat <<EOF >/home/"$BENUTZERNAME"/"$NEXTCLOUDDNS"/Nextcloud-Installationsskript/uninstall.sh
#!/bin/bash
if [ "\$(id -u)" != "0" ]
then
clear
...
rm -Rf /etc/ufw /etc/fail2ban /var/www /etc/mysql /etc/postgresql /etc/postgresql-common /var/lib/mysql /var/lib/postgresql /etc/letsencrypt /var/log/nextcloud /home/"$BENUTZERNAME"/"$NEXTCLOUDDNS"/Nextcloud-Installationsskript/install.log /home/"$BENUTZERNAME"/"$NEXTCLOUDDNS"/Nextcloud-Installationsskript/update.sh
...
exit 0
EOF
chmod +x /home/"$BENUTZERNAME"/"$NEXTCLOUDDNS"/Nextcloud-Installationsskript/uninstall.sh
##########################
# Re-Install. verhindern #
# Prevent Second Run     #
##########################
if [ -e "/var/www/$NEXTCLOUDDNS/config/config.php" ] || [ -e /etc/nginx/conf.d/$NEXTCLOUDDNS.conf ]; then
  clear
  echo "*************************************************"
  ...
  echo "* /home/"$BENUTZERNAME"/"$NEXTCLOUDDNS"/Nextcloud-Installationsskript/uninstall.sh"
  echo ""
  exit 1
else
  echo "*************************************************"
...
if [ ! -d "/home/"$BENUTZERNAME"/"$NEXTCLOUDDNS"/Nextcloud-Installationsskript/" ]; then
  echo "* Erstelle: Installationsskript-Verzeichnis: OK *"
  echo "* Creating: Install directory .......::::::: OK *"
  mkdir /home/"$BENUTZERNAME"/"$NEXTCLOUDDNS"/Nextcloud-Installationsskript/
  echo ""
  else
...
# install.log             #
###########################
exec > >(tee -i "/home/"$BENUTZERNAME"/"$NEXTCLOUDDNS"/Nextcloud-Installationsskript/install.log")
exec 2>&1
###########################
# D: Update-Funktion      #
# E: Update-function      #
...
function nextcloud_scan_data() {
  ${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ files:scan --all
  ${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ files:scan-app-data
  }
###########################
# D: Basissoftware        #
# E: Required software    #
...
###########################
${mkdir} -p /var/log/nextcloud /var/www/letsencrypt/.well-known/acme-challenge /etc/letsencrypt/$NEXTCLOUDDNS/rsa-certs /etc/letsencrypt/$NEXTCLOUDDNS/ecc-certs
...
###########################
${systemctl} restart php$PHPVERSION-fpm.service nginx.service
###########################
# Installation DB         #
###########################
${clear}
...
${cat} <<EOF >/etc/nginx/conf.d/$NEXTCLOUDDNS.conf
server {
    listen 80;
    listen [::]:80;
...
server {
  ...
  #ssl_certificate /etc/letsencrypt/${NEXTCLOUDDNS}/rsa-certs/fullchain.pem;
  #ssl_certificate_key /etc/letsencrypt/${NEXTCLOUDDNS}/rsa-certs/privkey.pem;
  #ssl_certificate /etc/letsencrypt/${NEXTCLOUDDNS}/ecc-certs/fullchain.pem;
  #ssl_certificate_key /etc/letsencrypt/${NEXTCLOUDDNS}/ecc-certs/privkey.pem;
  #ssl_trusted_certificate /etc/letsencrypt/${NEXTCLOUDDNS}/ecc-certs/chain.pem;
...
# Hostname                #
###########################
${sed} -i "s/server_name cloud.server.io;/server_name $(hostname) $NEXTCLOUDDNS;/" /etc/nginx/conf.d/$NEXTCLOUDDNS.conf
###########################
# Nextcloud-CRON          #
###########################
(/usr/bin/crontab -u www-data -l ; echo "*/5 * * * * /usr/bin/php -f /var/www/$NEXTCLOUDDNS/cron.php > /dev/null 2>&1") | /usr/bin/crontab -u www-data -
###########################
...
${apt} install -y bzip2
${tar} -xjf $NCRELEASE & CrI
mv nextcloud /var/www/$NEXTCLOUDDNS
...
if [ $DATABASE == "m" ]
then
sudo -u www-data php /var/www/$NEXTCLOUDDNS/occ maintenance:install --database "mysql" --database-name "${NCDBNAME}" --database-user "${NCDBUSER}" --database-pass "${NCDBPASSWORD}" --admin-user "${NEXTCLOUDADMINUSER}" --admin-pass "${NEXTCLOUDADMINUSERPASSWORD}" --data-dir "${NEXTCLOUDDATAPATH}"
else
sudo -u www-data php /var/www/$NEXTCLOUDDNS/occ maintenance:install --database "pgsql" --database-name "${NCDBNAME}" --database-user "${NCDBUSER}" --database-pass "${NCDBPASSWORD}" --admin-user "${NEXTCLOUDADMINUSER}" --admin-pass "${NEXTCLOUDADMINUSERPASSWORD}" --data-dir "${NEXTCLOUDDATAPATH}"
fi
${echo} ""
sleep 5
...
###########################
${sudo} -u www-data ${cp} /var/www/$NEXTCLOUDDNS/config/config.php /var/www/$NEXTCLOUDDNS/config/config.php.bak
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ config:system:set trusted_domains 0 --value="$YOURSERVERNAME"
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ config:system:set trusted_domains 1 --value="$NEXTCLOUDDNS"
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ config:system:set trusted_domains 2 --value="$IPA"
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ config:system:set overwrite.cli.url --value=https://"$NEXTCLOUDDNS"
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ config:system:set overwritehost --value="$NEXTCLOUDDNS"
${echo} ""
${echo} "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
${cp} /var/www/$NEXTCLOUDDNS/.user.ini /usr/local/src/.user.ini.bak
${sudo} -u www-data ${sed} -i 's/output_buffering=.*/output_buffering=0/' /var/www/$NEXTCLOUDDNS/.user.ini
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ background:cron
${sudo} -u www-data ${touch} /var/www/$NEXTCLOUDDNS/config/tweaks.config.php
${cat} <<EOF >>/var/www/$NEXTCLOUDDNS/config/tweaks.config.php
<?php
\$CONFIG = array (
...
EOF
${sed} -i 's/^[ ]*//' /var/www/$NEXTCLOUDDNS/config/config.php
###########################
...
${echo} ""
sleep 3
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:disable survey_client
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:disable firstrunwizard
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:disable federation
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:disable support
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:disable recommendations
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ config:app:set settings profile_enabled_by_default --value="0"
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:enable files_pdfviewer
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:enable contacts
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:enable calendar
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:enable groupfolders
if [ $NEXTCLOUDOFFICE == "y" ]
then
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:install richdocuments
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:install richdocumentscode
fi
if [ $ONLYOFFICE == "y" ]
then
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:install documentserver_community
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ app:install onlyoffice
fi
...
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ db:add-missing-primary-keys
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ db:add-missing-indices
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ db:add-missing-columns
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ db:convert-filecache-bigint
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ security:certificates:import /etc/ssl/certs/ssl-cert-snakeoil.pem
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ config:app:set settings profile_enabled_by_default --value="0"
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ config:app:set dav system_addressbook_exposed --value="no"
${sudo} -u www-data /usr/bin/php /var/www/$NEXTCLOUDDNS/occ config:app:set text workspace_available --value=0
${clear}
nextcloud_scan_data
${systemctl} restart nginx
${echo} ""
...
${echo} ""
${sudo} -u www-data /usr/bin/php -f /var/www/$NEXTCLOUDDNS/cron.php & CrI
###########################
...
/home/acmeuser/.acme.sh/acme.sh --issue -d "${NEXTCLOUDDNS}" --server letsencrypt --keylength 4096 -w /var/www/letsencrypt --key-file /etc/letsencrypt/${NEXTCLOUDDNS}/rsa-certs/privkey.pem --ca-file /etc/letsencrypt/${NEXTCLOUDDNS}/rsa-certs/chain.pem --cert-file /etc/letsencrypt/${NEXTCLOUDDNS}/rsa-certs/cert.pem --fullchain-file /etc/letsencrypt/${NEXTCLOUDDNS}/rsa-certs/fullchain.pem --reloadcmd "sudo /bin/systemctl reload nginx.service"
EOF
${sudo} -i -u acmeuser bash << EOF
/home/acmeuser/.acme.sh/acme.sh --issue -d "${NEXTCLOUDDNS}" --server letsencrypt --keylength ec-384 -w /var/www/letsencrypt --key-file /etc/letsencrypt/${NEXTCLOUDDNS}/ecc-certs/privkey.pem --ca-file /etc/letsencrypt/${NEXTCLOUDDNS}/ecc-certs/chain.pem --cert-file /etc/letsencrypt/${NEXTCLOUDDNS}/ecc-certs/cert.pem --fullchain-file /etc/letsencrypt/${NEXTCLOUDDNS}/ecc-certs/fullchain.pem --reloadcmd "sudo /bin/systemctl reload nginx.service"
EOF
${sed} -i '/ssl-cert-snakeoil/d' /etc/nginx/conf.d/$NEXTCLOUDDNS.conf
${sed} -i s/#\ssl/\ssl/g /etc/nginx/conf.d/$NEXTCLOUDDNS.conf
...
${echo} " » Passwordreset     : run "source /root/.bash_aliases" first or <nocc$REDISINDEX> won't work!"
${echo} " »                     nocc$REDISINDEX user:resetpassword $NEXTCLOUDADMINUSER"
${echo} ""
...
alias nocc$REDISINDEX="sudo -u www-data php /var/www/$NEXTCLOUDDNS/occ"
EOF
...
# Calculating runtime     #
###########################
mv $INSTALLATIONFILE $CONFIGFILE /home/"$BENUTZERNAME"/"$NEXTCLOUDDNS"/Nextcloud-Installationsskript/
${echo} ""
end=$(date +%s)
runtime=$((end-start))
echo ""
